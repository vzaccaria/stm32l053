/* Generated by the protocol buffer compiler.  DO NOT EDIT! */
/* Generated from: proto/message.proto */

/* Do not generate deprecated warnings for self */
#ifndef PROTOBUF_C__NO_DEPRECATED
#define PROTOBUF_C__NO_DEPRECATED
#endif

#include "proto/message.pb-c.h"
void   pct__init
                     (Pct         *message)
{
  static Pct init_value = PCT__INIT;
  *message = init_value;
}
size_t pct__get_packed_size
                     (const Pct *message)
{
  assert(message->base.descriptor == &pct__descriptor);
  return protobuf_c_message_get_packed_size ((const ProtobufCMessage*)(message));
}
size_t pct__pack
                     (const Pct *message,
                      uint8_t       *out)
{
  assert(message->base.descriptor == &pct__descriptor);
  return protobuf_c_message_pack ((const ProtobufCMessage*)message, out);
}
size_t pct__pack_to_buffer
                     (const Pct *message,
                      ProtobufCBuffer *buffer)
{
  assert(message->base.descriptor == &pct__descriptor);
  return protobuf_c_message_pack_to_buffer ((const ProtobufCMessage*)message, buffer);
}
Pct *
       pct__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data)
{
  return (Pct *)
     protobuf_c_message_unpack (&pct__descriptor,
                                allocator, len, data);
}
void   pct__free_unpacked
                     (Pct *message,
                      ProtobufCAllocator *allocator)
{
  assert(message->base.descriptor == &pct__descriptor);
  protobuf_c_message_free_unpacked ((ProtobufCMessage*)message, allocator);
}
static const ProtobufCFieldDescriptor pct__field_descriptors[3] =
{
  {
    "temp_value_C",
    1,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_FLOAT,
    0,   /* quantifier_offset */
    offsetof(Pct, temp_value_c),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "hum_value",
    2,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_FLOAT,
    0,   /* quantifier_offset */
    offsetof(Pct, hum_value),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
  {
    "pressure_value",
    3,
    PROTOBUF_C_LABEL_REQUIRED,
    PROTOBUF_C_TYPE_FLOAT,
    0,   /* quantifier_offset */
    offsetof(Pct, pressure_value),
    NULL,
    NULL,
    0,             /* flags */
    0,NULL,NULL    /* reserved1,reserved2, etc */
  },
};
static const unsigned pct__field_indices_by_name[] = {
  1,   /* field[1] = hum_value */
  2,   /* field[2] = pressure_value */
  0,   /* field[0] = temp_value_C */
};
static const ProtobufCIntRange pct__number_ranges[1 + 1] =
{
  { 1, 0 },
  { 0, 3 }
};
const ProtobufCMessageDescriptor pct__descriptor =
{
  PROTOBUF_C__MESSAGE_DESCRIPTOR_MAGIC,
  "pct",
  "Pct",
  "Pct",
  "",
  sizeof(Pct),
  3,
  pct__field_descriptors,
  pct__field_indices_by_name,
  1,  pct__number_ranges,
  (ProtobufCMessageInit) pct__init,
  NULL,NULL,NULL    /* reserved[123] */
};
